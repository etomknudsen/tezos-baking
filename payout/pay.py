...
for i in range(cycle_from, cycle_to):
        ...
        url_api = '{}/tables/income?cycle={}&address={}&columns=total_income,total_lost,delegated'.format(api_server, i, baker_pkh)
        ...
        url_api = '{}/tables/snapshot?delegate={}&cycle={}&is_selected=1&columns=address,balance,cycle'.format(api_server, baker_pkh, i)
        response = urllib.urlopen(url_api)
        data = json.loads(response.read().decode("utf-8"))
